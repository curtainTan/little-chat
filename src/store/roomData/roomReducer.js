...
function deleteRoom( state, action ){
    var joinedList = state.get( "joined" )
    var noJoinList = state.get( "noJoin" )
    var newState
    var getIndex = joinedList.findIndex( ( val ) => val.id === action.roomData.id  )
    if( getIndex !== -1 ){
    ...
    getIndex = noJoinList.findIndex( ( val ) => val.id === action.roomData.id  )
    if( getIndex !== -1 ){
...
function reciveMsg( state, data ){
    ...
    if( data.type !== "user" ){
        ...
        var index = joineList.findIndex( val => val.id === data.id )
        if( index !== -1 ){
            var count = state.getIn([ "user", index, "msgCount" ])
            // 判断是否是当前选中房间
